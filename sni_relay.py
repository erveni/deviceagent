...
# Upstream SOCKS5 the relay forwards to. Defaults to a local gost (anon/anon),
# but can be pointed straight at Decodo via env (gost's chain to Decodo MOBILE is
# broken — returns 0x03 — while a direct hostname-CONNECT to Decodo works, which
# is exactly what this relay does with rdns=True). See docs/MOBILE_PROXY_SETUP.md.
GOST_HOST = os.environ.get("SNI_UPSTREAM_HOST", "127.0.0.1")
GOST_USER = os.environ.get("SNI_UPSTREAM_USER", "anon")
GOST_PASS = os.environ.get("SNI_UPSTREAM_PASS", "anon")
GOST_PORT = int(os.environ.get("SNI_UPSTREAM_PORT", "0"))  # 0 => set from argv in __main__

SNI_PEEK_TIMEOUT = 5.0
UPSTREAM_TIMEOUT = float(os.environ.get("SNI_UPSTREAM_TIMEOUT", "15"))  # fast-fail per connect
UPSTREAM_RETRIES = int(os.environ.get("SNI_UPSTREAM_RETRIES", "2"))     # Decodo mobile drops bursts
BUF = 65536
...
def open_upstream(host: str, port: int) -> socket.socket:
    # Decodo mobile drops/times-out connection bursts (a page opens many sockets at
    # once). Fast-fail each connect and retry rather than hang the whole page on one.
    last = None
