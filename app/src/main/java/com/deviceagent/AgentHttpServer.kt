...
class AgentHttpServer(private val flowEngine: FlowEngine) {
    ...
    companion object {
        ...
        const val APP_VERSION_NAME = "0.9.70-copilot-network-guard-trial"
        const val APP_VERSION_CODE = 87
            ...
            for (platform in platformsOrder) {
                ...
                try {
                    ...
                    if (platform == "copilot") {
                        ...
                        if (cached && !step("verify_fresh_cached_conversation") { flowEngine.copilot.freshCachedConversation() }) {
                            pr.status = "error"; pr.error = "cache trial fresh conversation not verified"; continue
                        }
                        if (!step("input") { flowEngine.copilot.inputPrompt(prompt) }) {
                        ...
                        if (!step("wait_generation") { flowEngine.copilot.waitForAnswer(genTimeoutSec, confirmNetworkError = prepared) }) {
                            pr.status = "error"; pr.error = flowEngine.copilot.lastWaitFailure ?: "generation timeout"; continue
                        }
                        step("read_answer") {
