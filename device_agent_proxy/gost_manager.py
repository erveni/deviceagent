...
Usage:
    from device_agent_proxy.gost_manager import GostManager
...
# Exported launcher settings win; never load the sibling repository environment.
...
# Opt-in, process-local cache for verified Evomi targeting tiers.  The ranking
# dispatcher creates a GostManager for every attempt, including retries and OCR
# re-captures; repeatedly probing the same zip/city/region ladder consumes proxy
# traffic before the phone has even navigated.  Keep this disabled by default and
# short-lived: pool availability can change, so a cached tier must never become a
# permanent geo decision.
EVOMI_TIER_CACHE_TTL_S = float(os.environ.get("EVOMI_TIER_CACHE_TTL_S", "0"))
_evomi_tier_cache: Dict[tuple[str, str, str, str], tuple[float, str]] = {}
_evomi_tier_cache_lock = threading.Lock()
_cost_ledger_lock = threading.Lock()


def _parse_gost_metrics(payload: str) -> dict:
    """Extract cumulative service transfer counters without retaining labels.

    These counters update while sockets are open. Duplicate series are not summed
    twice; if client-specific series exist, ignore a service aggregate for the
    same direction, avoiding accidental aggregate-plus-components double counts.
    """
    series = {}
    pattern = re.compile(r'^gost_service_transfer_(input|output)_bytes_total'
                         r'\{([^}]*)\}\s+([^\s]+)(?:\s+\d+)?$')
    ...
    for line in payload.splitlines():
        ...
        if not match:
            continue
        direction, label_text, raw_value = match.groups()
        try:
            labels = {key: json.loads('"' + value + '"')
                      for key, value in label_pattern.findall(label_text)}
            service = labels.get('service', '')
            value = float(raw_value)
        except (ValueError, TypeError):
            continue
        if not re.fullmatch(r'service-\d+', service) or not math.isfinite(value) or value < 0:
            continue
        series[(direction, tuple(sorted(labels.items())))] = int(value)
    components = {(direction, dict(labels)['service']) for direction, labels in series
                  if 'client' in dict(labels)}
    totals = {}
    for (direction, items), value in series.items():
        labels = dict(items)
        service = labels['service']
        if (direction, service) in components and 'client' not in labels:
            continue
        total = totals.setdefault(service, {'input_bytes': 0, 'output_bytes': 0})
        total[direction + '_bytes'] += value
    return totals


def _summarize_gost_cost(lines) -> dict:
    """Count terminal SOCKS5 handler records once; never retain raw log fields.

    Connector records would double-count the same traffic. Connections still open
    when gost is terminated may have no terminal record, so this is a lower bound,
    not a replacement for the provider's billing meter. Duration is nanoseconds
    per connection (their sum is not elapsed wall time).
    """
    services = {}
    seen = set()
    for line in lines:
        try:
            ...
            continue
        if not isinstance(row, dict) or row.get("kind") != "handler" or row.get("handler") != "socks5":
            continue
        service, sid = row.get("service"), row.get("sid")
        if not isinstance(service, str) or not re.fullmatch(r"service-\d+", service) or not isinstance(sid, str) or not sid:
            continue
        values = [row.get(key) for key in ("inputBytes", "outputBytes", "duration")]
        ...
        if key in seen:
            continue
        seen.add(key)
        total = services.setdefault(service, {"connections": 0, "input_bytes": 0,
                                              "output_bytes": 0, "connection_duration_ns": 0})
        total["connections"] += 1
        total["input_bytes"] += values[0]
        total["output_bytes"] += values[1]
        total["connection_duration_ns"] += values[2]
    return services


def _evomi_tier_cache_key(spec: "ListenerSpec") -> tuple[str, str, str, str]:
    return (spec.country.lower(), spec.zip_code.strip(), evomi_slug(spec.city), evomi_slug(spec.region))


def _get_cached_evomi_tier(spec: "ListenerSpec") -> Optional[str]:
    if EVOMI_TIER_CACHE_TTL_S <= 0:
        return None
    key = _evomi_tier_cache_key(spec)
    with _evomi_tier_cache_lock:
        cached = _evomi_tier_cache.get(key)
        if not cached:
            return None
        stamped, tier = cached
        if time.monotonic() - stamped > EVOMI_TIER_CACHE_TTL_S:
            _evomi_tier_cache.pop(key, None)
            return None
        return tier


def _cache_evomi_tier(spec: "ListenerSpec", tier: str) -> None:
    # Country is the emergency fallback, not a positively geo-verified tier.
    if EVOMI_TIER_CACHE_TTL_S <= 0 or tier == "country":
        return
    with _evomi_tier_cache_lock:
        _evomi_tier_cache[_evomi_tier_cache_key(spec)] = (time.monotonic(), tier)
...
class GostManager:
    ...
    def _preflight_upstreams(self) -> None:
        ...
        for spec in self.specs:
            ...
            if PROXY_PROVIDER == "evomi":
                # Measurement-only fast path. A normal ranking attempt can spend
                # several proxy requests proving a zip before Chrome sends its
                # first byte. Direct zip preserves requested geography, but is
                # opt-in: an unsupported zip fails rather than widening silently.
                if os.environ.get("EVOMI_DIRECT_ZIP", "0") == "1" and spec.zip_code:
                    print(f"[gost]   {spec.device_id}: evomi direct zip={spec.zip_code} (preflight disabled)")
                    new_specs.append(replace(spec, tier="zip"))
                    self.mapping[spec.device_id]["tier"] = "zip"
                    continue
                cached_tier = _get_cached_evomi_tier(spec)
                if cached_tier:
                    print(f"[gost]   {spec.device_id}: evomi preflight cache hit "
                          f"tier={cached_tier} ttl={EVOMI_TIER_CACHE_TTL_S:.0f}s")
                    new_specs.append(replace(spec, tier=cached_tier))
                    self.mapping[spec.device_id]["tier"] = cached_tier
                    continue
                tiers = []
                ...
                # ZIP is an Evomi expert option with additional billed bandwidth.
                # Opt-in city-first preserves a verified city (not country-only),
                # falling back to the existing ZIP/region ladder if it has no pool.
                if os.environ.get("EVOMI_CITY_FIRST", "0") == "1" and spec.city:
                    tiers.sort(key=lambda candidate: candidate[0] != "city")
                picked_tier = "country"
                ...
                _cache_evomi_tier(spec, picked_tier)
    ...
    def start(self, wait_seconds: float = 1.5) -> None:
        ...
        self._phase_ledger = os.environ.get("GOST_PHASE_LEDGER", "").strip()
        if self._phase_ledger:
            # Loopback only; no upstream request is made by the metrics scraper.
            # The free port handoff has a small bind race; a metrics startup
            # failure is reported as missing telemetry, never assumed zero cost.
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as reservation:
                reservation.bind(("127.0.0.1", 0))
                self._metrics_port = reservation.getsockname()[1]
            args += ["-metrics", f"127.0.0.1:{self._metrics_port}"]
        log_path = cfg.name.replace(".yaml", ".log")
        self._log_path = log_path
        self._cost_started_at = time.time()
        self._cost_written = False
        ...
        if self._phase_ledger:
            self._phase_stop = threading.Event()
            self._phase_sample_lock = threading.Lock()
            self._phase_opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
            self._phase_thread = threading.Thread(target=self._sample_phase_loop,
                                                  name="gost-phase-metrics", daemon=True)
            self._phase_thread.start()
        time.sleep(wait_seconds)
        if self.process.poll() is not None:
            rc = self.process.returncode
            self.process = None
            if getattr(self, "_phase_stop", None) is not None:
                self._phase_stop.set()
                self._phase_thread.join(timeout=2)
                self._phase_stop = None
            try:
    ...
    def stop(self) -> None:
        # Capture open-connection traffic BEFORE terminating gost. This must
        # remain best-effort so a broken telemetry destination cannot leak proxy
        # processes or interrupt their cleanup.
        if getattr(self, "_phase_stop", None) is not None:
            self._phase_stop.set()
            worker = getattr(self, "_phase_thread", None)
            if worker is not None and worker is not threading.current_thread():
                worker.join(timeout=2)
            self.sample_phase_metrics("before_stop")
            self._phase_stop = None
    ...
    def _sample_phase_loop(self) -> None:
        stop = self._phase_stop
        while not stop.is_set():
            self.sample_phase_metrics("periodic")
            stop.wait(1)

    def sample_phase_metrics(self, phase: str = "phase") -> None:
        """Snapshot cumulative bytes; callers may tag phase boundaries explicitly.

        Default-off, no credentials or destination labels retained. Samples are
        cumulative and MUST be differenced, never summed across timestamps.
        """
        if not getattr(self, "_phase_ledger", ""):
            return
        sample_lock = getattr(self, "_phase_sample_lock", None)
        if sample_lock is None or not sample_lock.acquire(timeout=.6):
            return
        try:
            try:
                now = time.time()
                # Phase is an internal identifier, not arbitrary caller content.
                tag = phase if re.fullmatch(r"[a-zA-Z0-9_]{1,64}", phase) else "phase"
                record = {"schema": 1, "time": now, "pid": os.getpid(),
                          "gost_pid": self.process.pid if self.process else None,
                          "started_at": self._cost_started_at, "phase": tag,
                          "source": "gost_metrics_cumulative"}
                try:
                    with self._phase_opener.open(
                            f"http://127.0.0.1:{self._metrics_port}/metrics", timeout=.5) as response:
                        payload = response.read(2_000_000).decode("utf-8", errors="replace")
                    totals = _parse_gost_metrics(payload)
                    record["services"] = [
                        {"service": f"service-{index}", "device_id": spec.device_id,
                         "listener_port": spec.port, **totals[f"service-{index}"]}
                        for index, spec in enumerate(self.specs) if f"service-{index}" in totals]
                    record["status"] = "ok" if record["services"] else "no_counters"
                except Exception as exc:
                    record.update(status="unavailable", error_type=type(exc).__name__)
                with _cost_ledger_lock, open(self._phase_ledger, "a") as output:
            ...
            # Do not surface filesystem/URL exception strings containing paths
            # or credentials, and never obstruct the proxy lifecycle.
            pass

    def _write_cost_ledger(self) -> None:
        """Opt-in sanitized telemetry, serialized across threads and processes."""
        ledger = os.environ.get("GOST_COST_LEDGER", "").strip()
        log_path = getattr(self, "_log_path", None)
        if not ledger or not log_path or getattr(self, "_cost_written", False):
            return
        try:
            with _cost_ledger_lock:
                if getattr(self, "_cost_written", False):
                    return
                with open(log_path, errors="replace") as source:
                    totals = _summarize_gost_cost(source)
                ended = time.time()
                rows = []
                for index, spec in enumerate(self.specs):
                    total = totals.get(f"service-{index}", {"connections": 0, "input_bytes": 0,
                                                           "output_bytes": 0, "connection_duration_ns": 0})
                    rows.append(json.dumps({
                        "schema": 1, "pid": os.getpid(), "device_id": spec.device_id,
                        "listener_port": spec.port, "service": f"service-{index}",
                        "started_at": getattr(self, "_cost_started_at", None), "ended_at": ended,
                        "elapsed_s": max(0, ended - getattr(self, "_cost_started_at", ended)),
                        "source": "terminal_socks5_handler_lower_bound", **total,
                    }, separators=(",", ":")) + "\n")
                with open(ledger, "a") as output:
                    ...
                    try:
                        ...
                        fcntl.flock(output.fileno(), fcntl.LOCK_UN)
                self._cost_written = True
        except (OSError, ValueError) as exc:
            # Diagnostics must never prevent proxy teardown; no paths or raw
            # exception strings (which might include secrets) are logged.
            print(f"[gost-cost] ledger unavailable: {type(exc).__name__}", flush=True)
...
def self_test() -> int:
    ...
    try:
        ...
        print("[self-test] local SOCKS listener ready (credentials omitted)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        sys.exit(self_test())
    print("Usage: python3 -m device_agent_proxy.gost_manager test")
    sys.exit(1)
